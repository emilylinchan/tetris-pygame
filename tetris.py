...
class Tetromino:
    ...
    def __init__(self, x, y):
        ...
        rand_idx = random.randint(0, len(Tetromino.shapes) - 1) # Random shape and colour
        
        self.x = x
        self.y = y
        self.shape = Tetromino.shapes[rand_idx]
        self.colour = Tetromino.colours[rand_idx]
        self.rotation = 0 # Index value that corresponds to a particular orientation of the shape
...
class Tetris:
    ...
    def rotate(self):
        ...
        if self.collision():
            # Rotate the tetromino 360 degrees back to its original orientation if invalid move
...
def on_mouse_down(pos):
    ...
    if game.state == "start":
        ...
        # Check if any of the buttons were clicked on the options screen, return to start screen if so
